...
public class DownloadService extends Service {
    ...
    private static boolean patchError = false;
    ...
    private void startDownload(@NonNull UpdateConfig config, @Nullable IHttpManager httpManager, @Nullable UpdateCallback callback, @Nullable INotification notification) {
        ...
        String url = config.getApkUrl();
        String path = config.getSavePath();
        String filename = config.getSaveFilename();
        // 如果文件名为空则使用路径
    ...
    public static class AppDownloadCallback implements IHttpManager.DownloadCallback {
        ...
        private boolean isSupportNotifyCancelDownload;
        ...
        private AppDownloadCallback(Context context, DownloadService downloadService, UpdateConfig config, File apkFile, UpdateCallback callback, INotification notification) {
            ...
            this.isSupportNotifyCancelDownload = config.isSupportNotifyCancelDownload();
        ...
        public void onStart(String url) {
            ...
            if (isShowNotification && notification != null) {
                notification.onStart(context, notifyId, channelId, channelName, notificationIcon, getString(R.string.app_updater_start_notification_title), getString(R.string.app_updater_start_notification_content), config.isVibrate(), config.isSound(), isSupportNotifyCancelDownload);
            }
        ...
        public void onFinish(File file) {
            ...
            if (!TextUtils.isEmpty(config.getPatchUrl())) {//合成补丁
                String apkName = config.getSaveFilename();
                // 如果文件名为空则使用路径
                if (TextUtils.isEmpty(apkName)) {
                    apkName = AppUtils.getAppFullName(context, config.getApkUrl(), context.getResources().getString(R.string.app_name));
                }
                ...
                HPatch.getInstance().patchApk(oldApk.getAbsolutePath(), file.getAbsolutePath(), newApk.getAbsolutePath(), new HPatch.PatchCallback() {
                    ...
                    public void onPatchResult(boolean success) {
                        if (success){
                            ...
                            if (isOK) {
                                if (isShowNotification && notification != null) {
                                    notification.onFinish(context, notifyId, channelId, notificationIcon, getString(R.string.app_updater_finish_notification_title), getString(R.string.app_updater_finish_notification_content), newApk, authority);
                                }
                                // 下载完成
                                if (isInstallApk) {
                                    AppUtils.installApk(context, newApk, authority);
                                }
                                if (callback != null) {
                                    callback.onFinish(newApk);
                                }
                            }
                        } else {//错误
                            patchError = true;
                            ...
                            if (callback != null) {
                                callback.onError(new Exception("patch error"));
                            }
                        }
                    }
                });
